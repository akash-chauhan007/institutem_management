...
//import car from '../../assets/images/user/car.jpg'
 ...
 const BootstrapTable=()=>{
    ...
    // setHandleChange(brandName);
      console.log('>>>',e.target.value)
      const lena = await db.collection("IICS").where("brand", "==", brandName).get()
      // console.log(lena.docs.exists)
      if(lena.docs.length){
    ...
    //   .get() 
    //   const docs = lena.docs 
    
    }

      const fetchBlogs=async(e)=>{
              const ateenref = await  db.collection('IICS').where('cancel','==',false).get()        
    ...
    //   // const nababa = await db.collection("Ateen_India").doc(id).get()

    //  const lena = await db.collection("Ateen_India").where("brand", "==", brandName )
    //   .get() 
    //   const docs = lena.docs 
    //   let dataArray = docs.map(doc => doc.data())
    //  setHandleComp(dataArray)
    ...
    useEffect(() => {
     ...
     const change = await db.collection("IICS").doc(id)
     .update({
       "cancel":true
     });
      ...
      <div className="h-screen bg-light-100 dark:bg-dark-900 ">
        ...
        handleComp.map((item,mapId)=>{  
                                const {id, Roll_No, dprice1,Course, Name,FatherName, mrp1,BatchTime, screen_size, brand} = item
                                       ...
                                       {/* <Button variant="outline-primary"  onClick={() => setState({ isPaneOpen: true })}>
                                       Edit
                              </Button>                        */}
                              <td>
